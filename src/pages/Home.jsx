...
function songLabel(song) {
  ...
  return meta ? `${song.artist} - ${song.title} (${meta})` : `${song.artist} - ${song.title}`
}
...
export default function Home() {
  ...
  useEffect(() => {
    ...
    async function loadSongs() {
      ...
      try {
        ...
        const canUseWorker = typeof Worker !== 'undefined' && typeof Blob !== 'undefined'
        ...
        if (canUseWorker) {
          ...
          const songsFromWorker = await new Promise((resolve, reject) => {
            const timeout = setTimeout(() => reject(new Error('worker_timeout')), 15000)
          ...
          if (songsFromWorker?.ok && Array.isArray(songsFromWorker.songs)) {
            if (mounted) {
              ...
              setArtistOptionsAll(Array.isArray(songsFromWorker.artists) ? songsFromWorker.artists : [])
              setYearOptionsAll(Array.isArray(songsFromWorker.years) ? songsFromWorker.years : [])
              setStyleOptionsAll(Array.isArray(songsFromWorker.styles) ? songsFromWorker.styles : [])
            }
          } else {
          ...
          for (let i = 0; i < raw.length; i += chunkSize) {
            ...
            for (const item of slice) {
              ...
              if (normalized) {
                ...
                for (let s = 0; s < normalized.styles.length; s += 1) stylesSet.add(normalized.styles[s])
              }
            }
          ...
          if (mounted) {
            setSongs(list)
            setArtistOptionsAll(Array.from(artists).sort((a, b) => a.localeCompare(b)))
            setYearOptionsAll(Array.from(years).sort((a, b) => Number(b) - Number(a)))
            setStyleOptionsAll(Array.from(stylesSet).sort((a, b) => a.localeCompare(b)))
          }
        }
      } catch {
        if (mounted) setSongsError('Baza utworow nie zaladowala sie. Mozesz wpisac utwor recznie.')
      } finally {
  ...
  useEffect(() => {
    ...
    const t = setTimeout(async () => {
      ...
      const shouldSearch = (q.length >= minChars || hasFilters) && songs.length > 0
  ...
  }, [trackFinderOpen, songsLoading, songsError, deferredQuery, artistFilter, yearFilter, styleFilter, songs, isMobile])
            ...
            <div className="form">
              ...
              <div className="trackFinder">
                <div className="trackFinderHeader">
                  <div className="trackFinderTitle">Podpowiedzi z bazy utworow</div>
                ...
                {trackFinderOpen ? (
                  <>
                    ...
                    {searching && !songsLoading ? <div className="hint">Szukam...</div> : null}

                    {!songsLoading && !songsError ? (
                      <div className="songListWrap">
                        <div className="hint">
                          Znaleziono: <span className="mono">{filteredSongs.length}</span> | Pokazano:{' '}
                          <span className="mono">{displayedSongs.length}</span>
                        </div>
                        <div
                          className="songList"
                          ref={songListRef}
                          onScroll={(e) => setShowTopArrow(e.currentTarget.scrollTop > 180)}
                        >
                        {filteredSongs.length === 0 ? (
                          <div className="hint">
                            {trackQueryInput.trim().length < 2 && !artistFilter && !yearFilter && !styleFilter
                          ...
                          displayedSongs.map((song, idx) => (
                            <button
                              key={song.id}
                              className={`songItem songItemAnimate ${selectedSongId === song.id ? 'songItemActive' : ''}`}
                              type="button"
                            ...
                            >
                              ...
                              <div className="songMeta">
                                {song.year || '—'} | {song.styles.join(', ') || '—'} | {song.language || '—'}
                              </div>
                            </button>
                          ))
                        )}
                          <div ref={loadMoreRef} className="songListSentinel"></div>
                        </div>

                        {hasMoreSongs || loadingMore ? (
                          <div className="hint">{loadingMore ? 'Laduje wiecej...' : 'Przewin w dol, aby zaladowac wiecej.'}</div>
                        ) : null}

                        {showTopArrow ? (
                          <button className="backToTopBtn" type="button" onClick={scrollSongsTop}>
                            ↑
                          </button>
                        ) : null}
                      </div>
                ...
                <div className="pickerOverlay" role="dialog" aria-modal="true">
                  <div className="pickerSheet">
                    <div className="pickerHeader">
                      ...
                      <button className="btn btnSmall btnGhost" type="button" onClick={() => setArtistPickerOpen(false)}>
                        Zamknij
                      </button>
                    </div>
                    <input
                    ...
                    >
                      ...
                      {artistSuggestions.map((x) => (
                        <button
                          key={x}
                          type="button"
                          className={`pickerItem ${x === artistFilter ? 'pickerItemActive' : ''}`}
                          onClick={() => {
                            setArtistFilter(x)
                            setVisibleCount(PAGE_SIZE)
                            setArtistPickerOpen(false)
                ...
                <div className="pickerOverlay" role="dialog" aria-modal="true">
                  <div className="pickerSheet">
                    <div className="pickerHeader">
                      ...
                      <button className="btn btnSmall btnGhost" type="button" onClick={() => setStylePickerOpen(false)}>
                        Zamknij
                      </button>
                    </div>
                    <input
                    ...
                    >
                      ...
                      {styleSuggestions.map((x) => (
                        <button
                          key={x}
                          type="button"
                          className={`pickerItem ${x === styleFilter ? 'pickerItemActive' : ''}`}
                          onClick={() => {
                            setStyleFilter(x)
                            setVisibleCount(PAGE_SIZE)
                            setStylePickerOpen(false)
            ...
            <div className="status">
              {effectiveStatus === 'pending' ? (
                <>
                  ...
                  <div className="statusText">
                    Utwor: <span className="mono">{formatTrackLine(activeEntry)}</span>
                  </div>
                  <div className="statusText">
