...
export function getSortedQueue(queueMap, allowedStatuses) {
  const list = Object.entries(queueMap || {}).map(([id, value]) => ({
    id,
    ...value,
  }));
  return list
    .filter((x) =>
      allowedStatuses?.length ? allowedStatuses.includes(x?.status) : true
    )
    .sort((a, b) => {
      const ta = typeof a.createdAt === "number" ? a.createdAt : 0;
      const tb = typeof b.createdAt === "number" ? b.createdAt : 0;
...
// _____
export async function moveToPosition(entryId, newPosition) {
  if (!entryId) return;

  const queueSnap = await get(ref(db, "queue"));
  const queueMap = queueSnap.val() || {};

  const nowSnap = await get(ref(db, "state/nowId"));
  const nowId = nowSnap.val();

  if (nowId === entryId) {
    throw new Error('Nie mozna przesuwac elementu "Teraz"');
  }

  const active = getSortedQueue(queueMap, ["pending", "now"]);

  const currentIndex = active.findIndex((item) => item.id === entryId);
  if (currentIndex === -1) {
    throw new Error("Element nie znaleziony");
  }

  if (newPosition < 0 || newPosition >= active.length) {
    throw new Error("Nieprawidlowa pozycja");
  }
  ...
  const targetEntry = active[newPosition];
  const currentCreatedAt = queueMap[entryId].createdAt;
  const targetCreatedAt = queueMap[targetEntry.id].createdAt;
